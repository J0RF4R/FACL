(function (global) {
    ...
    var
        ...
        function addMouseoverListener(td, count, monthIndex) {
            td.addEventListener("mouseover", function() {
                mostrarInformacionAdicional(monthIndex, count);
            });
        }
    /**
    ...
    function createMonthTable(data, option) {
        ...
        // Crear la primera fila para las letras de los días
        for (c = 0; c <= 6; c++) {
        ...
        count = 1;
        var isTargetDateInCurrentMonth = (option.month === data.monthIndex && option.date === count);
        if (isTargetDateInCurrentMonth) {
            td.classList.add("dycalendar-target-date");
            td.addEventListener("mouseover", function() {
                mostrarInformacionAdicional(option.month, count);
            });
        }
        for (r = 0; r < 6; r++) {
            tr = document.createElement("tr");
            for (c = 0; c < 7; c++) {
                td = document.createElement("td");
                if (r === 0 && c < data.firstDayIndex) {
                    // Rellenar celdas vacías hasta el primer día de la semana
                    td.innerHTML = "";
                } else if (count <= data.totaldays) {
                    // Agregar el día del mes
                    td.innerHTML = count;
                    if (option.highlighttargetdates && isTargetDate(data.monthIndex, count, option.dates)) {
                        td.classList.add("dycalendar-target-date");
                        // Agregar evento mouseover para mostrar información adicional
                        addMouseoverListener(td, count, data.monthIndex);
                    }
                    if (data.today.date === count && data.today.monthIndex === data.monthIndex && option.highlighttoday) {
    ...
    function mostrarInformacionAdicional(monthIndex, day) {
        // Aquí puedes implementar la lógica para mostrar la tarjeta con información adicional
        // Por ejemplo, puedes crear un elemento div para la tarjeta y agregarlo al DOM con la información relevante
        var tarjeta = document.createElement("div");
        tarjeta.classList.add("informacion-adicional");
        tarjeta.innerHTML = "Información adicional para " + day + " de " + monthName.full[monthIndex];
        // Agregar la tarjeta al DOM, por ejemplo, al final del cuerpo del documento
        document.body.appendChild(tarjeta);
    }
    // Función auxiliar para verificar si una fecha es una fecha objetivo
    function isTargetDate(monthIndex, day, targetDates) {
        for (var i = 0; i < targetDates.length; i++) {
            if (targetDates[i].month === monthIndex && targetDates[i].date === day) {
                return true;
            }
        }
        return false;
